# Claude Max API Proxy (forked)

> **Fork of [claude-max-api-proxy](https://www.npmjs.com/package/claude-max-api-proxy)** with bug fixes and Linux systemd deployment support.

## Changes from upstream
- **Fix: array message content** (`dist/adapter/openai-to-cli.js`) — upstream assumed `msg.content` is always a string. OpenAI-compatible clients (including OpenClaw) send content as an array of parts `[{type:"text",text:"..."}]`, causing `[object Object]` in prompts. Added `extractText()` helper to handle both formats.
- **deploy/** — systemd service, wrapper script, and one-shot install script for Linux hosts (including SELinux/RHEL).

## OpenClaw configuration
```bash
openclaw config set models.providers.claude-max \
  '{"baseUrl":"http://localhost:3456/v1","apiKey":"not-needed","api":"openai-completions","models":[{"id":"claude-sonnet-4","name":"Claude Sonnet 4 (Max)"},{"id":"claude-opus-4","name":"Claude Opus 4 (Max)"},{"id":"claude-haiku-4","name":"Claude Haiku 4 (Max)"}]}'

openclaw config set agents.defaults.model \
  '{"primary":"claude-max/claude-sonnet-4","fallbacks":["openai-codex/gpt-5.3-codex"]}'
```

## Quick install (systemd)
```bash
git clone https://github.com/shaike1/claude-max-proxy
cd claude-max-proxy/deploy
# Edit claude-max-proxy.sh to set your NVM_NODE_PATH
bash install.sh
```
